          ...
          <Header/>
            <div class="pray screen">
              ...
              <div class="content_wrapper">
                ...
                <div class="content">
                  <p>Fellowship with us. God designed you to experience a full life with others in community. Encounter Jesus and grow together with others who love Him.</p>
                </div>
                <div class="prayer-functions">
                  <div class="function" v-for="(v1, index) in pray_list">
                    <div class="card" @click="navigationData(v1)" >
                      <img :src="v1.card_image" alt="" height="100%">
                    </div>
                  </div>
                </div>
                <button class="play-button" @click="navigationData(first_row)">
...
export default {
  ...
  mounted() {
    ...
    localStorage.removeItem('previoussUrl');
    this.felllowshipListing();
  },
  methods: {
    closeVideo(){
    ...
    navigationData(card){
      if(card.card_type === "group"){
      ...
      else if (card.card_type == "messages"){
         this.$router.push('messages');
      }
      else if (card.card_type == "friends") {
        this.$router.push('friend-page');
      }
      else{
        if (card.media_type == "video") {
          ...
          this.$router.push({
           name: "pdf",
            params: { id: card.id, section:'fellowship' }
             });
        }
       else if(card.media_type == "slider") {
          this.$router.push({
            name: "slider",
            params: { id: card.id, section: "fellowship" },
          });
