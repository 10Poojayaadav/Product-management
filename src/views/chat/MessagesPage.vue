<template>
  <v-app>
    ...
    <Header v-if="getCount" :get-count="getCount" />
    <Header v-else />
    <div class="messaging-zone">
      <div class="message-sidebar pane" id="left-pane" :style="{ maxWidth: leftPaneWidth + 'px' }">
        <div class="message-sidebar-inner">
          <div class="search-friend">
            ...
            <div class="friends">
              ...
              <template v-else>
                <div v-if="messageList.length > 0" class="list-view" id="list-view" v-on:scroll="scrollListView($event)">
                  <div  :class="chat.other_data
                        ? other_user_id == chat.other_data.id
                          ? 'frnd active'
                          : 'frnd'
                        : other_user_id == chat?.group_data?.group_id
                        ? 'frnd active'
                        : 'frnd'" v-for="(chat, index) in messageList"
                    :key="index"
                    @contextmenu="showMenu($event,chat.id)"
                  >
                    <div
                      v-if="chat.type == 'user'"
                    ...
                    <div class="group">
                      ...
                      <div class="info">
                          <h5 class="frnd-name text-white" v-if="chat.other_data !== null">
                            {{ chat.other_data.first_name }}
                            {{ chat.other_data.last_name }} 
                          </h5>
                          <h5 class="frnd-name" v-else>No Name</h5>
                          <p class="text-white" v-if="chat.message?.deleted_by != null && chat.message.deleted_by.includes(my_user_id)">
                          </p>
                          <p v-else class="text-white">
                            {{ chat.message?.get_msg_by?.id==my_user_id?'You':chat.message?.get_msg_by?.first_name+' '+chat.message?.get_msg_by?.last_name }}
                           {{ chat.message ? ':' : '' }}
                            {{ chat.message ? stripHtmlTags(chat.message?.message?.length<20 ? chat.message?.message : chat.message?.message?.substring(0,20)+"...") : '' }}
                          </p>
                      </div>
                    </div>
                    <button
                    ...
                    <span v-if="chat.unseen_count > 0" class="unseen-count"  >
                        <span class="count">{{ chat.unseen_count }}</span>
                    </span>
                     <div class="hamburger1">
                        <img src="/src/assets/ellipsis-vertical.svg" height="30" width="30">
                    </div>
                   <span v-if="userTypingId == chat.other_id" style="color: white; font-size: 12px; margin:auto;">&nbsp; &nbsp; typing...</span>
                    <div class="frnd-options">
                      <div class="options" :id="`option-id-${chat.id}`">
                        <ul>
                         <!--  <li @click="muteUserList(chat.other_id)">
                          ...
                          <li @click="deleteChatList(chat.id,chat.chatlistpin_count)">
                            Delete
                          </li>
                        </ul>
                      </div>  
                    ...
                    <div
                      ...
                      :id="`friend-${chat.group_data?.id}`" @click="
                            getChatDetail(
                          ...
                          <div class="info">
                            ...
                            <p v-if="chat.message" class="text-white">
                               <p v-if="chat.message.get_msg_by?.id == my_user_id">You: {{ stripHtmlTags(chat?.message?.message?.length<20 ? chat.message.message : chat?.message?.message?.substring(0,20)+"...") }}</p>
                                 <p v-else>{{ chat.message.get_msg_by?.first_name+' '+chat.message.get_msg_by?.last_name }}: {{ stripHtmlTags(chat.message.message.length<20 ? chat.message.message : chat.message.message.substring(0,20)+"...") }}</p>
                            </p>
                          </div>
                          <span
                      ...
                      <button
                        ...
                        <span  v-if="chat.unseen_count > 0" class="unseen-count"><span class="count">{{ chat.unseen_count }}</span></span>
                        <div class="hamburger1">
                        <img src="/src/assets/ellipsis-vertical.svg" height="30" width="30">
                    </div>
                    </div>
                ...
                <div v-else-if="!showLoaderOnList">
                  <div class="no-record-style">No records found!</div>
                </div>
      ...
      <!-- Chat Detail Extended View Start -->
           ...
           <div id="drag-bar" @mousedown="startDragging"></div>

        <div class="messages-container pane" @click="reactionPannelChecker($event)" id="right-pane">
          <div class="user-chat-info" v-if="nameHeader">
           ...
           <div v-if="moreLoad" class="chat-with_loader h-auto">
            ...
            >
                    ...
                    <span class="box-size-similar">
                      ...
                      <span v-else>
                        ...
                        >Attached
                         ...
                         <span v-else>{{ stripHtmlTags(message.get_msg_reply.message) }}</span>
                      </span>
                      ...
                      <div v-else class="user-name">
                        ...
                        <span class="reply-message-by-me" v-html="getText(message.message)"></span>
                      <!--   <span class="reply-message-by-me" style=" white-space: pre-wrap; word-wrap: break-word;">{{ getText(message.message) }}</span> -->
                      </span>
                      </span>
                      <span
                    ...
                    <span class="box-size-similar">
                      ...
                      <span
                        ...
                        <span class="attached-file-section" 
                          v-if="message.get_msg_reply.type == 'image' || message.get_msg_reply.type == 'document' || message.get_msg_reply.type == 'video' || message.get_msg_reply.type == 'audio'"
                        >Attached</span>
                        <span>{{stripHtmlTags(message.get_msg_reply.message) }}</span>
                      </span>
                      ...
                      <span class="reply-other-side-msg">
                       ...
                       <span> {{ getTime(message.created_at) }}</span></div>
                        <span class="reply-message-by-me" v-html="message.message"> </span>
                        <span
                          :id="`reaction-on-id-${message.id}`"
                        >
                          <template
                  ...
                  <div class="media image">
                    ...
                    <span
                      ...
                      <span
                        ...
                        class="reply-message-by-me" style="white-space: pre-line" v-html="getText(message.message)"
                        >
                      </span>
                      <span
              ...
              <div v-if="message?.type == 'audio'">
                ...
                <div class="message-actions">
                  ...
                  >
                    ...
                    <span
                      v-if="message.message != null"
                      class="chat-file-messag" style="white-space: pre-line;"
                      v-html="getText(message.message)">
                    </span>
                    <span
                      :id="`reaction-on-id-${message.id}`"
                    >
              ...
              <div
                v-if="message?.type == 'media' && message?.media_id" 
                :class="
                  message?.message_by == my_user_id && replyMsgId == ''
                    ? 'chat-image chat chat-me'
                    : 'chat-image chat chat-other'">
               <div class="message-actions">
                ...
                <span
                  ...
                  <div v-else class="user-name">
                    <span v-if="message.get_msg_to.id == my_user_id">You</span>
                    <span v-else>{{ message.get_msg_to.first_name }} {{ message.get_msg_to.last_name }}</span><span> {{ getTime(message.created_at) }}</span></div>
                    <span class="forward-msg" v-if="message.forward != null">
                      <img src="/src/assets/arrow.png" alt="" height="15" />
                      Forwarded
                    </span>
                    <img :src="message.media_data?.thumbnail" @dblclick="openFile(message.media_data)" />
                     <p v-if="message.media_data.media_type == 'pdf'">{{ parseMsg(message, "read") }}</p>
                     <p v-if="message.media_data.media_type == 'audio'">{{ parseMsg(message, "listen") }}</p>
                     <p v-if="message.media_data.media_type == 'video'">{{ parseMsg(message, "watch") }}</p>
                    <span
                    v-if="message.message != null"
                    class="chat-file-messag" style="white-space: pre-line;" v-html="getText(message.message)">
                  </span>
          ...
          <!-- share image or pdf -->
           <div class="image-preview" v-if="shareMedia" id="shareMedia">
            <div class="img-wrapper">
              <v-btn @click="closePreview">+</v-btn>
              <v-img :width="200" cover :src="mediaData.banner_image"  id="mediaImage"></v-img>
            </div>
            <!-- <button @click = "sharedFileUploadSend()">send</button> -->
          </div>
              ...
              <div :data-open="this.typographyActions" class="typography-actions">
                <ul>
                  ...
                  <li @click="editor.chain().focus().toggleStrike().run()">
                   <v-img src="/src/assets/strikethrough.png" height="12"></v-img>
                  </li>
...
export default {
       data() {
           return {
             ...
             moreLoad:false,
             isDragging: false,
             leftPaneWidth: 320, // Initial width of the left pane
             startX: 0, 
             message: '',
             arr:[],
             textareaHeight: 40,
             ...
             height:40,
             goTop: false,
              isReadySeenEvent: false,
             isPendingforMessageSeen: false,
  ...
  localStorage.removeItem('previousUrl');
   ...
   prevScrollTop = scrollTop;
    ...
    if (this.$route.query.user_id && !this.$route.query.media_id) {
      ...
      this.messagePage();
      axios
        .get("get-content/" + atob(this.$route.query.media_id))
        .then((response) => {
          this.sendMediaBtn = true;
          this.mediaData = response.data.data;
          this.mediaFile = response.data.data.media;
        })
        .catch(async (error) => {
          console.log("Error fetching data:", error);
          if(error.response.data.status_code == 401)
  ...
  methods: {
    parseMsg(chatLeft, subTitle) {
      // const msg = chatLeft.message ? `${chatLeft.message}\n` : "";
      const title = chatLeft.media_data && chatLeft.media_data.title ? `${this.capitalize(chatLeft.media_data.title)} ` : "";
      const makeSubTitle = subTitle ? `(${this.capitalize(subTitle)})` : "";
      const libPath = this.getLastStringInHierarchy(chatLeft.media_data ? chatLeft.media_data.library[0] : {});
      return `${title}${makeSubTitle}\n${this.capitalize(chatLeft.media_data.card_type.split("_")[0])}${libPath ? libPath : " > "}${title}`;
    },
    getLastStringInHierarchy(library, suffix = "") {
      const currentTitle = `${library.title ? library.title : ""}${suffix}`;
      if (library.library && library.library.length > 0) {
        const lastSubLibrary = library.library[library.library.length - 1];
        return this.getLastStringInHierarchy(lastSubLibrary, ` > ${currentTitle}`);
      }
 ...
 openPopup(id) {
    ...
    reactionPannelChecker(e){
      ...
      if(this.checker && !document.getElementById('right-pane').contains(e.target)){
        this.showReactionPicker = false;
        this.checker = false;
    ...
    clearUserGroupDetails() {
  this.userDetails = null;
  this.groupData = null;
  this.nameHeader = false; // Hide details section
},
    openAnyFileInput() {
      this.showReactionPicker = false;
   ...
   openFileInput() {
    ...
    handleFileUploadSend(event) {
      console.log("image upload", this.typeMessage);
      this.showSendButton = false;
      this.previewImage = null;
      var msg = this.typeMessage.replace(/<br\s*\/?>/gi, "\n");
    ...
    sharedFileUploadSend() {
      ...
      formData.append("file_msg", this.typeMessage);
      this.typeMessage = "";
      axios.post("user-store-message-web", formData)
        .then((response) => {
    ...
    replyMessage(event, msg) {
      ...
      if (msg.get_chat_file.length > 0) {
       ...
       const message = this.stripHtmlTags(msg.message);
       document.getElementById("reply-msg").textContent = message.substring(0,20)+"..";
      }
      document.getElementById("type_message").focus();
      document.getElementById("reply-msg-text").style.display = "block";

      document.getElementById("reply-user").textContent =
      msg.get_msg_by.id == this.my_user_id ? 'You' : msg.get_msg_by.first_name+' '+msg.get_msg_by.last_name;
      this.replyMsgIdCk = msg.id;
      event.preventDefault();
    },
    openFile(file_url) {
    if(file_url.media_type == "pdf")
    {
      console.log(file_url,"pooja yadav");
      const libraryTitle = file_url.library[0].title;
       this.$router.push({
            name: "library2",
            params: { id: file_url.library_id, section: "pray" },
            query: { pdfId: file_url.id , gridTitle:libraryTitle},
          });
    }
   else if(file_url.media_type == "video")
    {
        this.v_id=file_url.id;
        this.openVideo=true;
    }
   else if(file_url.media_type == "audio")
    {
          this.a_id=file_url.id;
          this.openAudio=true;
    }
    else{
    ...
    deleteChatList(id,chatCount) {
      this.currentPage = 1;
      if(chatCount > 0){ this.pinUserList(this.other_user_id,true); }
      axios
    ...
    scrollListView(event) {
      ...
      && this.currentPage != this.last_page &&  !this.Load) {
          this.Load = true;
          this.currentPage++;
          axios
            .post("user-chat-list-web",{page:this.currentPage})
            .then((response) => {
   ...
   async typedData(event) {
      ...
      content = content.replace(/<br class="ProseMirror-trailingBreak">/g, "").trim();
      // const plainTextWithLineBreaks = this.editor.getText({ preserveWhitespace: true });
      // this.typeMessage = plainTextWithLineBreaks;
      // this.typeMessage = content;

      content = this.editor.getText({ preserveWhitespace: true }); 
      content = content.replace(/&nbsp;/g, ' ').replace(/\n{2,}/g, '\n\n').trim(); 
      this.typeMessage = content;
      if (this.shareMedia === true) {
          if (( event.keyCode === 13) || (event.type === 'click' )) { 
            this.editor.commands.clearContent();
              this.sharedFileUploadSend();
              return;
          }
      }
      const sendMessage = async () => {
        if (this.typeMessage.trim() !== "" || this.selectedFile != null) {
            target.html('');
      ...
      if (event.keyCode === 13 && this.selectedFile != null) {
          ...
          if (event.shiftKey) {
              content = content.substring(caret, content.length);
              this.typeMessage = content.trimStart();
          } else {
              event.target.value = content?.substring(0, caret - 1) + content?.substring(caret, content.length);
              event.stopPropagation();
              event.preventDefault();
              this.selectText();
          ...
          if (event.shiftKey) {
              ...
              event.target.innerText = content?.substring(0, caret - 1) + content?.substring(caret, content.length);
      ...
      try {
          ...
          await axios.post("user-typing-status", {
              chat_type: this.userType,
              other_id: this.other_user_id,
          });
      } catch (error) {
   ...
   selectText() {
    ...
    updateUserDetailListNoLoad() {
      console.log("updateUserDetailListNoLoad");
      this.isLoading = false;
      axios
        .post("user-detail-message", {
          user_id: this.other_user_id,
    ...
    socketcall() {
      ...
      console.log("ChatDetailReaction event", this.page);
        axios
        .post("user-detail-message", {
          user_id: this.other_user_id,
          type: this.userType,
      ...
      this.$socket.on("ChatLara.App\\Events\\ChatInboxTyping", (event) => {
        console.log("ChatInboxTyping event");
    ...
    user() {
      axios
        ...
        .then((response) => {
          ...
          this.userAllInfo = response.data.data;
          this.onChatScreen = response.data.data.on_chat_screen;
        })
        .catch(async (error) => {
          if(error.response.data.status_code == 401)
          {
  ...
  storeMsg() {
    const msg = this.editor.getHTML();
    this.typeMessage = "";
    this.typographyActions = false;
    this.hideMediaIcons = true;
    ...
    if (this.replyMsgIdCk != "") {
      ...
      .post("user-store-message-web", {
        ...
        type: "text",
        other_id: this.other_user_id,
        message: msg, // Send the sanitized message
    ...
    axios
      ...
      .then((response) => {
        console.log("response after send message", response);
      })
      .catch(async (error) => {
        this.$toast.error(error.response.data.message, {
          duration: 4000, // Duration in milliseconds
...
.message-sidebar:hover #drag-bar {
  opacity: 1;
}
.chat-input {
  margin-top: 10px;
